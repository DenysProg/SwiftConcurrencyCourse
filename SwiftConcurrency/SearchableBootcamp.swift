...
final class RestaurantManager {
    func getAllRestaurants() async throws -> [Restaurant] {
        [
            Restaurant(id: "1", title: "Burgar Shack", cuisine: .american),
    ...
    private func addSubscribers() {
        $searchText
            ...
            .sink { [weak self] (searchText, searchScope) in
                self?.filterRestaurants(search: searchText, currentSearchScope: searchScope)
            }
            .store(in: &cancellables)
    }
    
    private func filterRestaurants(search: String, currentSearchScope: SearchScopeOption) {
